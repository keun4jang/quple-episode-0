...
**그룹명:** 쿼카 커플 (Quika Couple)
**세계관:** 혼자 떠나는 쿼카의 힐링 여행 모바일 캐주얼 게임의 메인 IP
...
| 카메라 위치 | 캐릭터 앞 = **-Y 쪽** |
| 얼굴 요소 | 머리의 -Y 표면에만 배치 |
| 카메라 각도 | 거의 수평(얼굴 응시), 모자 챙이 눈 가리지 않게 |
| 자동 검수 | 렌더 후 눈 하이라이트/코/볼 픽셀 카운트 → 미달 시 FAIL |

`render-quica-mascots.py`의 `verify_face()`가 매 렌더마다 얼굴 가시성을 자동 검증한다.
...
| ID | 이름 | 설명 | 우선순위 |
|---|---|---|---|
| `smile` | 기본 미소 | 현재 idle | 완료 |
| `travel_start` | 여행 출발 | 손 흔들며 앞으로 | High |
| `photo` | 카메라 촬영 | A가 카메라 들어올림 | High |
| `map_read` | 지도 보기 | 지도 펼쳐 응시 | High |
| `ticket_show` | 티켓 들기 | B가 티켓 자랑 | Medium |
| `star_found` | 별 발견 | 위 올려다보며 놀람 | Medium |
| `reward` | 보상 획득 | 두 손 들고 기뻐함 | Medium |
| `cheer` | 응원 | 팔 흔들기 | Medium |
| `surprise` | 놀람 | 눈 크게, 입 O | Low |
| `sad` | 실패/아쉬움 | 어깨 처짐 | Low |
| `nod` | 로딩 끄덕임 | 고개 위아래 | Low |
