...
**적** — 불안 / 욕심 / 불행 / 비교 / 번아웃 / 조급함 / 외로움 / 야근 귀신(보스)
- 맵을 떠다니다가 7m 안에 들어오면 쫓아오고, 닿으면 전투 시작 (`ShadowEnemy3D`)
- 전투 화면의 적은 12×12 픽셀 아트(`PixelArt`)로 그린다
- 조급함(Office3D)·외로움(CompanyLobby3D)은 `GameUI.SPAWN_TABLE`에 추가로 배치됨
...
## 장비 아이템 (상시 착용)
- `ItemDB`의 `kind: "equipment"` 아이템은 소모되지 않고 슬롯(`slot`)에 착용한다 — 지금은
  `scarf`(목도리, 방어 +4)와 `hat`(모자, 마음력 최대치 +10) 2종.
...
## 다음 작업 후보
- 그림자 감정 추가 더 (예: 미루기, 강박 등)
- 장비 아이템 더 추가 + 캐릭터 3D 모델에 시각적으로 착용 표시(현재는 스탯에만 반영, 외형 변화 없음)
- Android 빌드 설정
