...
- 지속 효과는 `_tick_statuses()`가 **적이 움직인 뒤 한 번** 흐른다. 약점을 찔러 적 턴을
  건너뛰면 상태 시간도 흐르지 않는다.
- 해제 수단: **캐모마일 차**(✦45) — 전투 중 나쁜 상태를 전부 없앤다. 나쁜 상태가 없으면 못 쓴다.
...
- `ItemDB`의 `kind: "equipment"` 아이템은 소모되지 않고 슬롯(`slot`)에 착용한다.
  ...
  - 꼬리: `tail_ribbon` 리본 꼬리끈(방어 +2, 마음력 최대치 +3, ✦60) /
    `star_charm` 별 장식 꼬리끈(마음의 힘 +3, 마음력 최대치 +3, ✦150, 외로움이 8% 드랍)
  - 상위 장비를 전부 착용하면 마음의 힘 +11, 방어 +8, 체력 +20, 마음력 +9 (총 ✦770어치)
...
- **3D 착용 표시**: 착용한 장비는 `PlayerQuokka3D`에 실제 메시로 그려진다. 아이템의 `wear`
  ...
  - 꼬리 = 꼬리를 감싸는 TorusMesh 띠 + 리본 고리(구) 또는 별 장식(구 + 십자 막대)
