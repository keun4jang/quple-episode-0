...
- `ItemDB`의 `kind: "equipment"` 아이템은 소모되지 않고 슬롯(`slot`)에 착용한다.
  ...
  - **둘이서 등급** (파트너 합류 뒤 상점에 나온다 — `requires_partner`):
    `sun_hat` 나란한 밀짚모자(마음력 +8, 방어 +2, ✦185) /
    `pair_muffler` 같이 두른 목도리(방어 +9, 체력 +8, ✦195) /
    `hold_gloves` 손잡는 장갑(마음의 힘 +6, 마음력 +2, ✦205) /
    `pair_sandals` 나란한 샌들(체력 +24, 방어 +2, ✦215) /
    `knot_charm` 둘이 묶은 매듭(마음의 힘 +4, 방어 +2, ✦190)
