...
- `ItemDB`의 `kind: "equipment"` 아이템은 소모되지 않고 슬롯(`slot`)에 착용한다.
  슬롯은 `scarf`(목)·`hat`(머리) 2개이고, 자리마다 기본/상위 장비가 하나씩 있다:
  - 목: `scarf` 포근한 목도리(방어 +4, ✦70) / `star_scarf` 별무늬 목도리(방어 +7, ✦150, 강박이 드물게 떨군다)
  - 머리: `hat` 털모자(마음력 최대치 +10, ✦65) / `travel_cap` 여행 모자(마음력 최대치 +6, 마음의 힘 +2, ✦140)
- 같은 슬롯의 장비를 착용하면 기존 것이 자동으로 해제된다. 가방 카드에 `[목]`/`[머리]`로 자리를 표시한다.
...
- 가방 UI에 "장비" 섹션이 별도로 있고 착용/해제 버튼으로 조작한다. 상점(자판기)에서도 구매 가능.
- **3D 착용 표시**: 착용한 장비는 `PlayerQuokka3D`에 실제 메시로 그려진다. 아이템의 `wear`
  필드(`color`/`accent`/`style`)를 읽어 목에는 TorusMesh 링+자락, 머리에는 SphereMesh 크라운
  +CylinderMesh 테두리(+방울 또는 챙)를 만든다. `PlayerStats.equipment_changed` 시그널이 오면
  `_refresh_equipment()`가 메시를 다시 만든다 — 착용/해제/불러오기 모두 즉시 반영된다.
