...
## InventoryUI — 인벤토리. 소모품과 중요 물품만 들어있다.
## 장비는 장비 창, 퀘스트는 퀘스트 창으로 완전히 분리했다.
...
func _refresh_content() -> void:
    ...
    for id in PlayerStats.inventory.keys():
        if ItemDB.is_equipment(id):
            continue  # 장비는 장비 창에서 다룬다
        if ItemDB.is_story_item(id):
            story_items.append(id)
        else:
            consumables.append(id)

    content.add_child(make_label("— 소모품 —", 30, Color("#6A5A4A")))
    ...
    content.add_child(make_label("— 중요 물품 —", 30, Color("#6A5A4A")))
    if story_items.is_empty():
        content.add_child(make_label("아직 챙긴 물품이 없어요.", 26, Color("#8A7A6A")))
