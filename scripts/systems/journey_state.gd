...
## **가까워졌다고 볼 만한 칸.** 엽서와 말 전하기가 이 선을 본다.
##
## 여태 둘 다 `HEART_MAX`(다섯)를 요구했는데, 마음이 느는 길이
## "하루 한 번 말 걸기" 하나뿐이라 그건 사실상 **한 사람에게 닷새
## 출석하기**였다. 마을 할 일은 하루 이틀이면 끝나니 평범하게 놀면
## 두세 칸에서 떠난다 — 그래서 말 전하기 일곱 쌍도, 엽서도, 처음부터
## 끝까지 걸어 본 시뮬레이션에서 **한 번도 안 나왔다.** 아무도 못 보는
## 콘텐츠를 벽 뒤에 쌓아 둔 셈이었다.
##
## 선을 셋으로 낮추고, 그 대신 마음이 느는 길을 늘렸다 —
## 마을을 다 돌아보면, 전언을 전해 주면, 노을을 같이 보면 한 칸씩.
## **출석이 아니라 같이 한 일이 마음을 채운다.**
## 다섯 칸은 그대로 둔다. 꽉 찬 자리는 여전히 있어야 한다.
const HEART_CLOSE := 3
...
const LETTERS := [
	...
	# **여기로 당겨 왔다.** 여태는 열넷째라 `due = arrivals/2` 로는
	# 아홉 마을을 한 번씩만 도는 보통 플레이(도착 9회, due 4)로는
	# 영영 안 왔다 - "재회가 이 게임의 심장" 이라면서 그 상대의 편지가
	# 죽은 콘텐츠였다. 앞의 엄마 다섯 통 순서는 그대로 둔다.
...
## 여행지를 다녀올 때마다 살핀다. 도착할 때마다 한 통.
##
## 가라고 시키지 않는 대신 편지가 쌓인다. 화살표도 느낌표도 안 쓴다.
## 세는 것이 **다녀온 곳 수가 아니라 도착 횟수**인 이유:
## 1탄에 여행지가 넷뿐이라 `places_visited()` 는 아무리 다녀도 4 에서 멈춘다.
## 같은 곳에 다시 가는 것도 떠난 것이니, 떠난 횟수로 센다.
##
## **두 도착에 한 통이던 것을 한 도착에 한 통으로 당겼다.** 아홉
## 마을을 한 번씩만 도는 보통 플레이는 도착이 아홉 번뿐이라, 예전
## 속도로는 편지가 넷(전부 엄마)에서 멈췄다 - 스물한 통 중 재회
## 상대(너구리)와 마을 인연 태반이 죽은 콘텐츠였다. 이제 그 아홉
## 번으로 아홉 통까지 온다.
## 보낸 사람 이름 → folk_id. **안 만난 이의 편지는 아직 안 보낸다** —
## 모르는 사람에게서 편지가 오면 다정한 게 아니라 이상하다. 버리지는
## 않는다: 만나고 나면 다음 차례에 온다. 필수 인연·가족은 늘 보낸다.
## **만난 적 있는 사람의 편지만 온다.** 여태는 서브 인연 넷에만 걸려
## 있어서, 가풀재에 가 본 적도 없는데 국수집 아저씨가 "국물은 늘 있으니
## 생각나면 와" 라고 편지를 보냈다. 규칙("모르는 사람에게서 편지가 오면
## 다정한 게 아니라 이상하다")은 진작 있었는데 절반에만 걸려 있었다.
##
## 가족(엄마·아빠·동생)은 뺀다 — 만난 적이 있고 없고를 따질 사이가 아니다.
## 아직 못 만난 이의 편지는 사라지지 않고 미뤄졌다가, 만나고 나면
## 다음 도착 때 밀린 채로 온다 (`maybe_letter` 의 `letters_skipped`).
const LETTER_NEEDS_MEET := {
	"돌 위의 수달": "gu_otter", "솔숲의 다람쥐": "so_squirrel",
	"물가의 개구리": "ba_frog", "갈대 사이의 고라니": "ga_deer",
	"가게 할머니": "seal", "갈매기 소년": "seagull",
	"빵집 아주머니": "ju_seal", "능 지키는 아이": "ju_kid",
	"국수집 아저씨": "san_seal", "부두 청년": "san_gull",
	"귤 파는 할머니": "do_seal", "자전거 탄 아이": "do_kid",
	"배낭 멘 너구리": "raccoon",
}
...
## **그 사람이 쓴 것처럼.** 위의 열두 줄은 누구에게나 맞는 말이라
## 아무나 쓸 수 있었다 — 빵집 아주머니와 자전거 탄 아이가 받은 순서에
## 따라 같은 문장을 보냈다. 편지(`LETTERS`)는 진작 사람마다 달랐는데
## 엽서만 공용이었다. 엽서는 **마음 칸을 채운 상대**에게서 오는 것이라
## 오히려 더 그 사람다워야 한다.
##
## 쓰는 법: 제 자리에서 오늘 있었던 일 한 가지 + 붙잡지 않는 한 마디.
...
const POSTCARD_BY_FOLK := {
	...
	# 볕뉘
	"ju_seal": "오늘 빵은 좀 탔어요. 그래도 다 팔렸고요.",
	"ju_kid": "오늘도 바람 소리만 났어요. 심심하진 않아요.",
	# 가풀재
	"san_seal": "계단은 여전히 많아요. 국물도 여전하고.",
	"san_gull": "배가 늦게 들어왔어요. 기다리는 것도 나쁘지 않더라고요.",
	# 하늬섬
	"do_seal": "귤이 이번엔 좀 셔. 그래도 다들 사 가더라.",
	"do_kid": "십사 분으로 줄였어요. 자랑은 아니고요.",
	# 굽이나루
	"gu_otter": "그 돌, 아직 거기 있어요. 조금 더 매끈해졌고요.",
	"gu_gull": "강이 저 굽이까지 넓어졌어요. 바다 같진 않지만요.",
	"cap_guinaru": "강물은 오늘도 같은 자리에서 굽어요. 지날 일 있으면 들르고.",
	# 방울못
	"ba_frog": "비 온 날 밤에 제일 크게 울었어요. 듣는 사람은 없었지만요.",
	"ba_gull": "어젯밤 연못에 별이 가득 담겼어요. 데크 한 바퀴면 다 보이는데.",
	"cap_bangul": "오늘도 많이는 안 구웠어요. 그래서 다 나갔고요.",
	# 갈밭머리
	"ga_deer": "갈대가 키를 넘었어요. 이제 잘 안 보여서 좋아요.",
	"ga_gull": "전망대에서 보면 하늘이 아직도 조각나 보여요.",
	"cap_galbat": "바람 센 날이 이어져요. 여긴 늘 이렇지만.",
	# 솔은재
	"so_squirrel": "묻어 둔 자리를 하나 잊었어요. 봄에 싹이 나면 그게 답이겠죠.",
	"so_gull": "고갯마루에 또 올라가 봤어요. 오늘은 다 작아 보이진 않더라고요.",
	"cap_sol": "올라오는 사람이 뜸해요. 앉을 자리는 늘 비어 있고.",
	# 꽃눈벌
	"kk_magpie": "꽃눈이 두 개 열렸어요. 어제는 하나였는데.",
	"kk_gull": "물도 없는데 또 왔어요. 도랑 소리가 그래도 물 같아서요.",
	"cap_kkot": "밭 아홉 뙈기 다 돌고 앉았어요. 오늘은 여기까지.",
	# 잿마루 (프롤로그). 여기 사람도 마음을 채우면 소식을 보낸다.
	"coworker": "자리는 그대로예요. 아무도 안 앉고요.",
	"guard": "밤에 불 켜진 층이 좀 줄었어. 좋은 건지는 모르겠고.",
	# 다시 만나는 그 사람. 재회로 채워진 마음이라 결이 달라야 한다.
	"raccoon": "지금 어디쯤이에요? 저도 제가 어디쯤인지 몰라요. 그래도 괜찮더라고요.",
}
...
func give_postcard(folk_id: String, who: String) -> void:
	...
	# 그 사람 것이 있으면 그걸 쓰고, 없으면 공용 줄을 **받은 순서**대로
	# 돌린다. 이름 해시로 고르던 때는 12줄로 늘려도 우연히 여섯 명이 같은
	# 줄에 몰렸다. 글은 받는 순간 저장되므로 옛 엽서는 안 바뀐다.
	var text := String(POSTCARD_BY_FOLK.get(folk_id, ""))
	if text == "":
...
##
## **저녁은 17시가 아니라 18시로 건너뛴다.** `day_part()` 의 문턱은
## 17시 그대로지만(그 전 표시·조건은 안 바뀐다), 가로등·등대 불은
## `night_amount()` 가 17시 반부터 스미듯 켜지도록 되어 있다 - 정확히
## 17시로 건너뛰면 하늘은 아직 낮처럼 밝고 등대에도 불이 하나도 없어서,
## "불 켜진 등대 사진 남기기" 라는 약속의 장면을 못 본 채 매듭이 끝났다.
## 18시면 확실히 불이 들어온 뒤다.
const DAY_PART_START := {"아침": 0.0, "낮": 11.0 * 60.0, "저녁": 18.0 * 60.0}
