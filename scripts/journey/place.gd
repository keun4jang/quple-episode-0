...
## 하루를 끝내는(또는 낮잠으로 미뤄 두는) 유일한 방법이다 —
## 밤을 새워도 벌이 없고, 그냥 밤 풍경을 계속 본다.
## 잘 자리에 섰다. **오늘 저녁에만 되는 일**이 남았으면 한 번만 알린다.
##
## 자면 12시간이 지난다(`JourneyState.sleep()`) — **늘 아침이 되는
## 건 아니다.** 자정을 넘기면 그만큼만 다음 날로 넘어가고, 안 넘기면
## 같은 날 그대로 시간만 흐른다(낮잠). 저녁에만 되는 것을 남겨 둔 채
## 자정을 넘겨 자면 그 저녁이 통째로 사라진다. 막지는 않는다. 알고
## 자는 것과 모르고 자는 것만 가른다.
##
...
func go_to_sleep() -> void:
	...
	tw.tween_callback(func():
		...
		# 화면이 깜깜한 지금이 자리를 옮길 때다. `JourneyState.sleep()`
		# 이 12시간만 넘기므로 낮잠이면 아직 오늘 저녁일 수도 있다 —
		# **"아침 자리로" 가 아니라 그때그때 시간대(`day_part()`) 자리로.**
		_relocate_folk(true))
...
func put_wanderer(sheet: String, who: String, folk_id: String,
	...
	if JourneyState.is_reunion(place_name()):
		...
		#
		# **셋째 재회(제목 대사)를 지나면 이 한 줄이 마을마다 달라진다.**
		# 낱말 하나만 바꾼다 — "낯익은"(그저 알아보는 것)에서
		# "반가운"(알아보고 반가운 것)으로. 마을 붙박이 대사를 전부
		# 새로 쓰는 대신, 아홉 마을이 공유하는 이 한 줄만 손대면 "이
		# 순간 이후로는 어디서든 다르다"는 게 제일 넓게 퍼진다.
		if hud != null:
			var seen := "반가운 얼굴이 보여요." if JourneyState.reunions >= 3 \
				else "낯익은 얼굴이 보여요."
			hud.call("_say_hint", seen, true, 3.0)
